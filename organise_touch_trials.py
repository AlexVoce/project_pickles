...
# --------------------------------------------------------------------------
# Optional pynapple layer: continuous TsdTensor + IntervalSet per condition
# --------------------------------------------------------------------------

def build_pynapple_view(organised: organisedDataset, touchDict: dict,
                         zF_n_time: Optional[int] = 120, npix_n_time: Optional[int] = 400,
                         zF_time_axis: Optional[int] = None,
                         npix_time_axis: Optional[int] = None) -> organisedDataset:
    """
    Reconstructs each trial's window in *absolute* time (trialOnT[i] + relative
    time) and concatenates all trials into one continuous nap.TsdTensor per
    signal (zF, npix_ds), plus a nap.IntervalSet per trial. Condition ->
    IntervalSet mappings are stored in organised.condition_intervals so you can
    do e.g.:

        organised.tsdtensor_zF.restrict(organised.condition_intervals['iCueL_1port_cor'])

    which returns only the samples belonging to that condition's trials, with
    the plotting/restrict/bin_average niceties of pynapple.

    Requires `pip install pynapple`.
    """
    if not HAVE_PYNAPPLE:
        raise ImportError("pynapple is not installed. `pip install pynapple` to use this.")

    sp = touchDict['seshParams']
    trialOnT = np.asarray(sp['trialOnT'])
    n_trials = len(trialOnT)
    time_rel = organised.time_zF  # shared grid for both signals after downsampling

    zF = np.asarray(touchDict['zF_touchMat'])
    npixMat = np.asarray(touchDict['npix_touchMat'])
    npixMat = npixMat/100 # 
    zF, npixMat = orient_trial_time_feature(
        zF, npixMat, n_trials,
        zF_n_time=zF_n_time, npix_n_time=npix_n_time,
    ...
    npix_ds = downsample_along_axis(npixMat, axis=1, target_n=len(time_rel))

    dt = time_rel[1] - time_rel[0]
    win_dur = time_rel[-1] - time_rel[0]
    # guard against overlapping trial windows (would break monotonic concatenation)
    min_gap = np.min(np.diff(trialOnT)) if n_trials > 1 else np.inf
    if min_gap < win_dur:
        print(f"[organise_touch_trials] WARNING: trial windows ({win_dur:.2f}s) may overlap "
              f"(min inter-trial gap {min_gap:.2f}s). Continuous concatenation may not be "
              "strictly monotonic; consider a narrower `window`.")

    all_t, all_zF, all_npix = [], [], []
    trial_starts, trial_ends = [], []
    for i in range(n_trials):
        abs_t = trialOnT[i] + time_rel
        all_t.append(abs_t)
        all_zF.append(zF[i])
        all_npix.append(npix_ds[i])
        trial_starts.append(abs_t[0])
        trial_ends.append(abs_t[-1])

    all_t = np.concatenate(all_t)
    all_zF = np.concatenate(all_zF, axis=0)
    all_npix = np.concatenate(all_npix, axis=0)

    # enforce strictly increasing time (required by pynapple); if trials
    # overlap this will drop the offending samples with a warning above.
    order = np.argsort(all_t, kind='stable')
    all_t, all_zF, all_npix = all_t[order], all_zF[order], all_npix[order]
    keep = np.concatenate([[True], np.diff(all_t) > 0])
    if not keep.all():
        print(f"[organise_touch_trials] Dropping {np.sum(~keep)} non-increasing "
              "timestamp(s) from concatenation (likely overlapping trial windows).")
    all_t, all_zF, all_npix = all_t[keep], all_zF[keep], all_npix[keep]

    # data here is (time, n_features) i.e. 2D -> TsdFrame. If your zF/npix
    # arrays actually carry an extra spatial dimension per timestep (e.g.
    # time x height x width), use nap.TsdTensor instead of nap.TsdFrame below.
    if all_zF.ndim == 2:
        tsdtensor_zF = nap.TsdFrame(t=all_t, d=all_zF)
        tsdtensor_npix = nap.TsdFrame(t=all_t, d=all_npix)
    else:
        tsdtensor_zF = nap.TsdTensor(t=all_t, d=all_zF)
        tsdtensor_npix = nap.TsdTensor(t=all_t, d=all_npix)

    trial_intervals = nap.IntervalSet(start=np.array(trial_starts), end=np.array(trial_ends))

    condition_intervals = {}
    for cond_name, idx in touchDict['trialTypeInds'].items():
        idx = np.asarray(idx, dtype=int)
        if idx.size == 0:
            condition_intervals[cond_name] = nap.IntervalSet(start=[], end=[])
        else:
            condition_intervals[cond_name] = trial_intervals[idx]

    organised.tsdtensor_zF = tsdtensor_zF
    organised.tsdtensor_npix = tsdtensor_npix
    organised.condition_intervals = condition_intervals
    return organised
...
def plot_condition_comparison(organised: organisedDataset, conditions: list,
                               feature_idx: int = 0, signal: str = 'zF', ax=None):
    """Quick trial-averaged +/- SEM comparison plot across conditions for one feature (neuron/ROI)."""
    import matplotlib.pyplot as plt
    if ax is None:
        fig, ax = plt.subplots(figsize=(6, 4))
    t = organised.time_zF
    for cond_name in conditions:
        cond = organised[cond_name]
        if len(cond.trial_idx) == 0:
            continue
        if signal == 'zF':
            m, s = cond.mean_zF()[:, feature_idx], cond.sem_zF()[:, feature_idx]
        else:
            m, s = cond.mean_npix()[:, feature_idx], cond.sem_npix()[:, feature_idx]
        ax.plot(t, m, label=f"{cond_name} (n={len(cond.trial_idx)})")
        ax.fill_between(t, m - s, m + s, alpha=0.2)
    ax.axvline(0, color='k', linestyle='--', linewidth=0.8)
    ax.set_xlabel("Time from trial-on (s)")
    ax.set_ylabel(f"{signal} (feature {feature_idx})")
    ax.legend(fontsize=8)
    return ax


if __name__ == "__main__":
    # Minimal smoke test with synthetic data shaped like the real touchDict,
    # so you can sanity-check the pipeline before pointing it at real data.
    rng = np.random.default_rng(0)
    n_trials = 40
    n_features = 5
    seshParams = {
        'trialOnT': np.sort(rng.uniform(0, 2000, n_trials)),
        'cueSide': rng.choice(['LEFT', 'RIGHT'], n_trials),
        'portSide': rng.choice(['LEFT', 'RIGHT'], n_trials),
        'rewardSide': rng.choice(['LEFT', 'RIGHT'], n_trials),
        'preT': 1, 'postT': 1, 'baseSec': 1, 'plotT': 2,
    }
    n_neurons = 195
    touchDict_demo = {
        'seshParams': seshParams,
        # per-NEURON label (e.g. microzone), length == n_neurons, NOT n_trials
        'iZone': rng.integers(0, 5, n_neurons),
        # (trial, feature, time) layout, with DIFFERENT feature counts between
        # zF and npix -- matches the real touchDict (e.g. 195 neurons vs 61
        # touch channels), to make sure detection doesn't rely on them matching.
        'zF_touchMat': rng.normal(size=(n_trials, n_neurons, 120)),
        'npix_touchMat': rng.normal(size=(n_trials, 61, 400)),
        'trialTypeInds': {
            'iCueL_1port_cor': np.where(seshParams['cueSide'] == 'LEFT')[0][:10],
            'iCueR_1port_cor': np.where(seshParams['cueSide'] == 'RIGHT')[0][:10],
        },
    }
    organised = organise_dataset(touchDict_demo)
    organised.summary()
    print("zone ids:", organised.zone_ids())
    print("zF for zone 0, condition iCueL_1port_cor:", organised.zF_by_zone('iCueL_1port_cor', 0).shape)
    build_pynapple_view(organised, touchDict_demo)
    print("pynapple TsdTensor (zF):", organised.tsdtensor_zF)
